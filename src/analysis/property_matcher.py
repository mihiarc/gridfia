...
class PropertyMatcher:
    ...
    def __init__(self, min_size_ratio=0.6, max_size_ratio=1.4, distance_threshold=2000):
    ...
    def find_all_matches(self, heirs_gdf: gpd.GeoDataFrame, all_properties: gpd.GeoDataFrame, 
        ...
        """Find matches for multiple heir properties.
        ...
        for idx, heir in heirs_sample.iterrows():
        ...
        if duplicate_matches.any():
            print(f"Warning: Found {duplicate_matches.sum()} duplicate matches")
...
def main():
    ...
    try:
        ...
        print(f"Found {len(vance_properties)} properties in Vance County")
        
        if len(vance_properties) == 0:
            print("No Vance County properties found")
            return
        ...
        # Print match summary
        print("\nMatching summary:")
        print(f"Average matches per property: {len(matches) / matches['heir_id'].nunique():.1f}")
        print(f"Average distance: {matches['distance'].mean():.1f}m")
        print(f"Average area ratio: {matches['area_ratio'].mean():.2f}")
